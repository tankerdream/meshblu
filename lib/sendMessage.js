...
function MessageSender(socketEmitter, mqttEmitter, parentConnection){
  ...
  //向多个设备发送多条信息
  var sendMessages = function(devices, data, token, topic, fromUuid, fromDevice, toDevices, callback){
    async.each(devices, function(device, done){
      sendMessage(device, data, token, topic, fromUuid, fromDevice, data.devices, done);
    }, callback);
  };

  var sendMessage = function(device, data, token, topic, fromUuid, fromDevice, toDevices, callback){
    ...
    getDevice(device, function(error, check) {
      ...
      //验证发送消息的安全性
      s_securityImpl.canSend(fromDevice, check, token, function(error, permission){
  ...
  MessageSender.prototype.hyga_sendMessage = function(fromDevice, data, topic, callback){
    ...
    return sendMessages(devices, data, token, topic, fromUuid, fromDevice, data.devices, callback);
