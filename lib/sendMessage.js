...
function createMessageSender(socketEmitter, mqttEmitter, parentConnection){
    ...
    getDevice(device, function(error, check) {
      ...
      s_securityImpl.canSend(fromDevice, check, function(error, permission){
        ...
        if(!permission){
          clonedMsg.UNAUTHORIZED=true; //for logging
          forwardMessage(clonedMsg);
          return callback(hyGaError(401,'No permission'));
        }
  ...
  return function(fromDevice, data, topic, callback){
    ...
    data = _.clone(data);
    if(!data.devices) {
      callback(hyGaError(401,'Invalid Message Format'));
      return;
    }
    ...
    debug(benchmark.toString())
    async.parallel([
      async.apply(broadcastMessage, data, topic, fromUuid, fromDevice),//广播消息,data.devices可能是字符串
      //单点发送消息,devices是数组,也可能是'device1/device2'形式的单元素数组
