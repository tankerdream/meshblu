...
function MessageSender(socketEmitter, mqttEmitter, parentConnection){
  ...
  //向多个设备发送多条信息
  var sendMessages = function(uuids, data, token, topic, fromUuid, fromDevice, toDevices, callback){
    async.each(uuids, function(uuid, done){
      sendMessage(uuid, data, token, topic, fromUuid, fromDevice, data.devices, done);
    }, callback);
  };

  var sendMessage = function(uuid, data, token, topic, fromUuid, fromDevice, toDevices, callback){
  ...
  MessageSender.prototype.hyga_sendMessage = function(fromDevice, data, topic, callback){
    ...
    return sendMessages(uuids, data, token, topic, fromUuid, fromDevice, data.devices, callback);
