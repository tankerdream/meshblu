...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('register', function (data) {
    ...
    s_register(s_channel,params, function(error, device){
      ...
      if(error){
        return socket.emit('register',{code:error.code,error: error.message});
      } else {
  ...
  socket.on('data', function(data) {
    ...
    s_authorizeRequest(data, function(err, fromDevice){
      if(err){
        return socket.emit('data',{error: err.message});
      }
      skynet.sendActivity(getActivity('data', socket, fromDevice));
      s_saveDataIfAuthorized(skynet.sendMessage, fromDevice, data, function(error,res){

        if(error){
          return socket.emit('data',{error: error.message});
        }
  ...
  socket.on('ssubscribe', function(data) {
    ...
    s_authorizeRequest(data, function(err, fromDevice){
      ...
      subscribeAndForward(fromDevice, res,data.params.uuid, data.params.token);
  ...
  socket.on('ssubscribe', function(data) {
    ...
    s_authorizeRequest(data, function(err, fromDevice){
      ...
      subscribeAndForward(fromDevice, res,data.params.uuid, data.params.token,data.params.type,false);
