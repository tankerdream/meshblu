...
function getDevice(socket, callback) {
  ...
  if(socket.hygaDevice){
    whoAmI(socket.hygaDevice.uuid, function(error, device) {
...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('identity',function (data) {
    ...
    s_authorizeRequest(data, function(error, fromDevice){
      ...
      s_updateSocketId(data, function(error, auth){
        ...
        //初始订阅received类消息
        debug(socket.id, 'subscribing to received', auth.uuid);
        socket.messageIOClient.subscribe(auth.uuid);
  ...
  socket.on('postData', function(data,fn) {
    fn = fn || _.noop;
    ...
    saveData(socket.hygaDevice, data, function(error){
