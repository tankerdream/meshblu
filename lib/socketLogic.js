...
function socketLogic (socket, secure, skynet){
  ...
  //注册设备
  socket.on('sregister', function (data) {
    ...
    s_register(owner,params, function(error, device){

      if(error){
        return socket.emit('register',{error: error.message});
      } else {
  ...
  socket.on('sdata', function(data) {
    ...
    s_authorizeRequest(data, function(err, fromDevice){
      ...
      skynet.sendActivity(getActivity('data', socket, fromDevice));
      s_saveDataIfAuthorized(skynet.sendMessage, fromDevice, data.uuid, data.params, function(error,saved){

        if(error){
          return socket.emit('data',{error: error.message});
        }

        return socket.emit('data',saved);
