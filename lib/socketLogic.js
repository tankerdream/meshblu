...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('message', function(data,fn) {
    ...
    message.subdevice = data.subdevice;
    message.topic = data.topic;

    //调用sendMessage文件中最后return的function
    skynet.sendMessage(socket, message,null,function(error){

      if(error){
        return fn(error);
      }

      return fn({devices:data.devices, subdevice: data.subdevice, payload: data.payload});
