...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('data', function(data) {
    ...
    s_authorizeRequest(data, function(error, fromDevice){
      ...
      s_saveDataIfAuthorized(skynet.sendMessage, fromDevice, data, function(error){

        if(error){
          return s_emitError(socket,error);
        }

        return socket.emit('data',201);
  ...
  socket.on('unsubscribe', function(data) {
    ...
    fn = fn || _.noop;
    s_authorizeRequest(data, function(error){
      ...
      if (!socket.messageIOClient) {
        return s_emitError(socket,{error: "socket not initialized"});
      }

      socket.messageIOClient.unsubscribe(data.uuid, data.types);

      socket.emit('config',{"api": "unsubscribe", "uuid": data.uuid});
