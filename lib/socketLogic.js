...
var s_saveDataIfAuthorized = require('./saveKey');
...
var s_getPublicKey = require('./s_getKey');
var s_getData = require('./s_getData');
...
function getDevice(socket, callback) {
  //防止socket建立之后设备信息再次改变,故再次进入数据库进行查询
  if(socket.skynetDevice){
    whoAmI(socket.skynetDevice.uuid, function(error, device) {
      return callback(error, device);
    });
  }else{
    return callback(new Error('HyGa device not found for socket' + socket), null);
  }
}

function s_getDevice(socket, callback) {
  //防止socket建立之后设备信息再次改变,故再次进入数据库进行查询
  if(socket.skynetDevice){
    ...
    return callback(hyGaError(404,'HyGaDevice not found for socket'), null);
  }
}

//验证设备请求的合法性
function s_authorizeRequest(data,callback){
...
function responseMsg(fn, error, data){
...
function socketLogic (socket, secure, skynet){
  ...
  var notInitialized = hyGaError(401,'Socket not initialized');

  socket.ipAddress = ipAddress;
  logEvent(100, {socketid: socket.id, protocol: 'websocket'});

  //socket.emit('identify'); //为meshblu的客户端配合,简化建立连接的步骤,我们不需要


  socket.on('identity',function (data) {

    s_authorizeRequest(data, function(error, fromDevice){

      if(error){
        return socket.emit('notReady', error);
      }
      ...
      data = {
        uuid: data.uuid,
        ipAddress: ipAddress,
        online: socket.auto_set_online
      };

      s_updateSocketId(data, function(error, auth){
        if (error){
          return socket.emit('notReady', error);
        }
        ...
        socket.messageIOClient.on('message', function(message){
          debug(socket.id, 'relay message', message);
          socket.emit('msg', message);
        });

        socket.messageIOClient.on('data', function(message){
          debug(socket.id, 'relay data', message);
        ...
        socket.messageIOClient.subscribe(auth.device.uuid);
  ...
  socket.on('register', function (data,fn) {
    ...
    s_register(data, function(error, device){

      responseMsg(fn, error, device);

    });

  });

  socket.on('ip', function (data,fn) {
    fn = fn || _.noop;

    if(notInitialized){
      return fn(notInitialized);
    }

    fn({code:201,ip:socket.ipAddress});

  });

  socket.on('whoAmI', throttler.throttle(function (fn) {
    fn = fn || _.noop

    if(notInitialized){
      return responseMsg(fn, notInitialized);
    }

    getDevice(socket, function(error, device){
      return responseMsg(fn, error, device);
    });
  ...
  socket.on('devices', function(data,fn) {
    ...
    s_getDevice(socket, function(err, device){
      if(err){ return fn(err); }

      s_getDevices(device, data, function(error, result){
        if(error){
          return fn(error);
        }
        var res = {code:201};
  ...
  socket.on('device', function(data,fn) {
    ...
    s_getDevice(socket, function(err, device){
      ...
      s_getOneDevice(device, data, function(error, result){
        if(error){
          return fn(error);
        }
        var res = {code:201};
  ...
  socket.on('resetToken', function(message, fn){
    ...
    s_getDevice(socket, function(err, fromDevice){
      if(err){ return fn(err); }
  ...
  socket.on('msg', function(data,fn) {
    ...
    if(typeof data !== 'object'){ return responseMsg(fn, hyGaError(401,'Invalid Message Format')); }

    getDevice(socket, function(error, fromDevice){

      if(error){
        return responseMsg(fn, error);
      }

      skynet.sendMessage(fromDevice, data, null, function(error){
        responseMsg(fn, error);
      });
  ...
  socket.on('subscribe', function(data,fn) {
    ...
    if(!data) {
      return fn(hyGaError(401,'Not enough information to subscribe'));
    }

    //这类型消息客户端都是以message类型接收,这三类相当于message的子类
    //var requestedSubscriptionTypes = data.types || ['broadcast', 'received', 'sent'];
    var requestedSubscriptionTypes = data.types || ['broadcast', 'received'];

    if(!_.isArray(requestedSubscriptionTypes)){
      requestedSubscriptionTypes = [requestedSubscriptionTypes]
    }
    ...
    debug('requestedSubscriptionTypes',requestedSubscriptionTypes);

    socket.messageIOClient.subscribe(data.uuid, requestedSubscriptionTypes, data.topics);
  ...
  socket.on('unsubscribe', function(data, fn) {
    ...
    fn = fn || _.noop;
    var data = data || {};
    var uuid = data.uuid || socket.skynetDevice.uuid;

    if(notInitialized){
      return fn(notInitialized);
    }

    //TODO 两边同时登陆,发送全部信息到config订阅端
    socket.messageIOClient.unsubscribe(uuid, data.types);
  ...
  socket.on('data', function(data,fn) {
    ...
    s_saveDataIfAuthorized(skynet.sendMessage, socket.skynetDevice, data, function(error){
  ...
  socket.on('getdata', function(data, fn) {
    ...
    data.params.channelUuid = socket.skynetDevice.owner;
    ...
    s_getData(data, function(results){
  ...
  socket.on('getPublicKey', function(uuid, fn){
    fn = fn || _.noop;
    ...
    s_getPublicKey(uuid, function(error, publicKey){
  ...
  socket.on('update', function (data, fn) {
    ...
    s_getDevice(socket, function(error, fromDevice){

      if(error){ return fn(error); }

      updateFromClient(fromDevice, data, function(error, regData) {
  ...
  socket.on('updateList', function (data, fn) {
    ...
    s_getDevice(socket, function(error, fromDevice){
      ...
      s_updateList(fromDevice, data, function(regData) {
  ...
  //meshblu代码
  socket.on('sdisconnect', function (data) {
  ...
  socket.on('sregister', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
  ...
  socket.on('supdate', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
    if(!data){
      data = {};
    }
  ...
  socket.on('sdata', throttler.throttle(function(data, fn) {
    fn = fn || _.noop
    getDevice(socket, function(err, fromDevice){
      if(err){ return; }
  ...
  socket.on('sgetdata', throttler.throttle(function(data, fn) {
    fn = fn || _.noop
  ...
  socket.on('smessage', throttler.throttle(function (message, fn) {
    fn = fn || _.noop
    if(typeof message !== 'object'){ return fn({error: 'Invalid Message Format'}); }
  ...
  socket.on('sgetPublicKey', throttler.throttle(function(uuid, fn){
  ...
  socket.on('sresetToken', throttler.throttle(function(message, fn){
    fn = fn || _.noop
    getDevice(socket, function(err, fromDevice){
      if(err){ return; }
  ...
  socket.on('sgenerateAndStoreToken', throttler.throttle(function(message, fn){
    getDevice(socket, function(err, fromDevice){
      if(err){ return logError(err); }
      generateAndStoreToken(fromDevice, message, function(error, result){
        if(error) {
