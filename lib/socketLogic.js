...
var s_generateAndStoreToken = require('./s_token');
...
function getDevice(socket, callback) {
  ...
  if(socket.skynetDevice){
    ...
    return callback(new Error('HyGaDevice not found for socket' + socket), null);
  }
}
...
function s_authorizeRequest(data,callback){
  ...
  authDevice(uuid, token, function (error, device) {
    ...
    if(!device){
      return callback("{code:401,{error:'Unauthorized'}}");
    }
...
function socketLogic (socket, secure, skynet){
  ...
  socket.on('identity',function (data,fn) {
    ...
    s_authorizeRequest(data, function(error, fromDevice){
      ...
      s_updateSocketId(data, function(auth){
        ...
        //Announce presence online告知所有设备设备上线
  ...
  socket.on('register', function (data,fn) {
    ...
    //TODO channel 改为devToken
    var s_channel = data.channel || {};

    var params = data.params;

    s_register(s_channel,params, function(error, device){
  ...
  socket.on('generateAndStoreToken', function(message, fn){
    getDevice(socket, function(err, fromDevice){
      ...
      s_generateAndStoreToken(fromDevice, message, function(error, result){
        if(error) {
  ...
  socket.on('message', function(data,fn) {
    ...
    if(typeof data !== 'object'){ return fn("{code:401,{error: 'Invalid Message Format'}}"); }
    ...
    getDevice(socket, function(error, fromDevice){
      ...
      skynet.sendMessage(fromDevice, data, null, function(error,res){
        if(error){
          return fn(error);
        }
        return fn({code:201});
  ...
  //meshblu代码
  socket.on('sidentity', throttler.throttle(function (data) {
    socket.auto_set_online = data.auto_set_online !== false;
    if (socket.auto_set_online) {
      data = _.extend({}, data, {
        socketid: socket.id,
  ...
  socket.on('ssubscribe', throttler.throttle(function(message, fn) {
  ...
  socket.on('sunsubscribe', throttler.throttle(function(data, fn) {
    fn = fn || _.noop;
  ...
  socket.on('sdevice', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
  ...
  socket.on('sdevices', throttler.throttle(function (data, fn) {
    fn = fn || _.noop
