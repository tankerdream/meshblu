...
function authorizeRequest(req, callback){
  var uuid = _.find(req.options, {name:'36'});
  var token = _.find(req.options, {name:'37'});
  if (uuid && uuid.value) {
...
function setupCoapRoutes(coapRouter, skynet){
  ...
  //获取uuid所对应的设备信息
  coapRouter.get('/sdevice', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
  ...
  coapRouter.get('/devices', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
      ...
      s_getDevices(fromDevice, req.query, false, function(error,data){
  ...
  //删除设备
  coapRouter.delete('/sdevice', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
  ...
  // coap get coap://localhost/whoami
  coapRouter.get('/swhoami', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
