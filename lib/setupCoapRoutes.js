...
function setupCoapRoutes(coapRouter, skynet){
   ...
   * 注册设备,将设备注册至mongodb中.
   */
  coapRouter.post('/sdevices', function (req, res) {

    req.params.ipAddress = req.params.ipAddress || req.rsinfo.address;
    var owner = {}
    owner.uuid = req.params.owner
    owner.psd = req.params.psd

    s_register(owner,req.params, function (error, data) {
      if(error) {
        res.statusCode = 500;
  ...
  coapRouter.post('/sdata/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      s_saveDataIfAuthorized(skynet.sendMessage, fromDevice, req.params.uuid, req.params, function(error, saved){
        res.json(saved);
      });
  ...
  //获取路径中uuid的sent类型的消息
  coapRouter.get('/ssubscribe/:uuid/sent', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('subscribe', req, fromDevice));
      logEvent(204, {fromUuid: fromDevice.uuid, from: fromDevice, uuid: req.params.uuid});
  ...
  //获取路径中uuid的broadcast类型的消息
  coapRouter.get('/ssubscribe/:uuid/broadcast', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('subscribe', req, fromDevice));
      logEvent(204, {fromUuid: fromDevice.uuid, from: fromDevice, uuid: req.params.uuid});
  ...
  //获取路径中uuid的received类型的消息
  coapRouter.get('/ssubscribe/:uuid/received', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('subscribe', req, fromDevice));
      logEvent(204, {fromUuid: fromDevice.uuid, from: fromDevice, uuid: req.params.uuid});
  ...
  coapRouter.post('/smessages', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
  ...
  coapRouter.put('/sdevices/:uuid', function (req, res) {
    authorizeRequest(req, res, function(fromDevice){
