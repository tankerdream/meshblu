...
function hyga_response(res, error, result){
  var message = {};
  if (error) {
...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/register/:channelUuid', function(req, res){
    var params = {}
    params.ipAddress = getIP(req);
    params.regToken = req.body.regToken;

    var channelUuid = req.params.channelUuid;

    s_register(channelUuid, params, function(error, device){
  ...
  server.post('/10mToken/:uuid', function(req, res){
    s_getAuthDeviceFromRequest(req, function(error, fromDevice){
