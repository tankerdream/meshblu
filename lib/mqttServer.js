...
var mqttServer = function(config, parentConnection){
  ...
  server.on('published', function(packet, client) {
    ...
    if('msg' === topic){
      ...
      saveData(client.skynetDevice, msg, function(error){

        hyga_emitClient(client.skynetDevice.uuid, 'ack', error, null, callbackId);

      });
    }
    else if('putKey' === topic){

      saveKey(client.skynetDevice, msg, function(error){
        hyga_emitClient(client.skynetDevice.uuid, 'ack', error, null, callbackId);
      });

    }
    else if('getKey' === topic){

      s_getKey(client.skynetDevice, msg, function(error, key){
        hyga_emitClient(client.skynetDevice.uuid, 'ack', error, key, callbackId);
      });

    }
    else if('brd' === topic){
      debug('broadcast', msg);
      broadcast(client.skynetDevice, msg, function(error, resp){
        return hyga_emitClient(client.skynetDevice.uuid, null, error, resp, callbackId);
      });
    }
    else if('subBrd' === topic){
      debug('subBroadcast', msg);
      client.messageIOClient.subBroadcast(client.skynetDevice, msg.uuids, function(error, resp){
        return hyga_emitClient(client.skynetDevice.uuid, null, error, resp, callbackId);
      });
    }
    else if('unsubBrd' === topic){
