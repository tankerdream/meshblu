...
var mqttServer = function(config, parentConnection){
  ...
  function authenticate(client, uuid, token, callback) {
    ...
    s_authorizeRequest(uuid, token,function(error,device) {
      ...
      s_updateSocketId(data, function(err, auth){
        if(error){
        ...
        client.messageIOClient.on('broadcast', function(message){
          ...
          hyga_emitClient(client.skynetDevice.uuid,'broadcast',null, message);
        });
  ...
  server.on('published', function(packet, client) {
    ...
    if('message' === topic){
      ...
      sendMessage(client.skynetDevice, msg, null, function(error, resp){
        return hyga_emitClient(client.skynetDevice.uuid, 'resp', error, resp, callbackId);
      });
    }
      ...
      client.messageIOClient.subBroadcast(client.skynetDevice, msg.devices, function(error, resp){
        return hyga_emitClient(client.skynetDevice.uuid, 'resp', error, resp, callbackId);
      });
    }
    else if('unsubBroadcast' === topic){
      debug('unsubBroadcast', msg);
      client.messageIOClient.unsubBroadcast(client.skynetDevice, msg.devices, function(error, resp){
        return hyga_emitClient(client.skynetDevice.uuid, 'resp', error, resp, callbackId);
      });
    }
    else if('update' === topic){
      updateFromClient(client.skynetDevice, msg, function(error, data){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, null, callbackId);
      });
    }
    else if('updateList' === topic){
      s_updateList(client.skynetDevice, msg, function(error, data){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, null, callbackId);
      });
    }
    else if('device' === topic){
      s_getOneDevice(client.skynetDevice, msg, function(error, data){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, data, callbackId);
      });
    }
      ...
      getToken(client.skynetDevice, msg, function(error, result){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, result, callbackId);
      });
    }
    else if('getPublicKey' === topic) {
      getPublicKey(client.skynetDevice, msg.uuid, function(error, publicKey){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, {publicKey: publicKey}, callbackId);
      })
    }
    else if('whoAmI' === topic){
      whoAmI(client.skynetDevice.uuid, true, function(error, resp){
        hyga_emitClient(client.skynetDevice.uuid, 'resp', error, resp, callbackId);
      });
    }
