...
var mqttServer = function(config, parentConnection){
  ...
  function hyga_emitClient(uuid, type, error, message, request){
    ...
    delete message.qos;
    if(error){
      return emitToClientDirectly(uuid, {type: 'error', success: false, payload: error, _callbackId: request.callbackId}, {qos: qos});
    }
    return emitToClientDirectly(uuid, {type: type, success: true, payload: message, _callbackId: request.callbackId}, {qos: qos});
  }
